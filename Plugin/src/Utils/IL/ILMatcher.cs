...
internal class LdargMatcher(int? arg) : ILMatcher
{
    ...
    public bool Matches(CodeInstruction instruction) =>
        arg.HasValue ? instruction.GetLdargIndex() == arg : instruction.GetLdargIndex().HasValue;
}
...
internal class LdlocMatcher(int? loc) : ILMatcher
{
    ...
    public bool Matches(CodeInstruction instruction) =>
        loc.HasValue ? instruction.GetLdlocIndex() == loc : instruction.GetLdlocIndex().HasValue;
}
...
internal class StlocMatcher(int? loc) : ILMatcher
{
    ...
    public bool Matches(CodeInstruction instruction) =>
        loc.HasValue ? instruction.GetStlocIndex() == loc : instruction.GetStlocIndex().HasValue;
}
